sap.ui.define([
	...
	return BaseController.extend("vn.pp.ux_vn_pp_config.controller.FinSrvComps_001", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf vn.pp.ux_vn_pp_config.view.FinSrvComps_001
		 */
		onInit: function () {
			//Groups ID a Actualizar el primero es el principal
			this.aGroupID = ["FinSrvComps_001", "FinSrvComps_001_new"];
			//Nombre modelo a actualizar
			this.sModelName = "modeloConfig";
			//Nombre de tablas a actualizar
			this.sObjectName = "tableFinSrvComps_001";
			//Nombre agregaciones a actualizar
			this.sObjectAggregation = "rows";
			//Nombre principal propiedad a actualizar:
			this.sMainProperty = "/FinSrvComps_001_new";
			//Eventos
			var omcb = this.byId("cbcompanyCode_FinSrvComps_001");
			omcb.clearSelection();
		...
		onSearch: function (oEvent) {
			...
			var aFilters = [];
			aFilters.push(that.getTableFiltersMCB(oView, "cbcompanyCode_FinSrvComps_001", "companyCode", false));
			aFilters.push(that.getTableFiltersCB(oView, "cbFinCode_FinSrvComps_001", "finServ_finCode", false));
			if (aFilters[0] === false) {
