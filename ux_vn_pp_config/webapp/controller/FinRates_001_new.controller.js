		...
		onSave: function (oEvent) {
			...
			MessageBox.confirm(that.getText("msgConfirmTxt"), {
				...
				onClose: function (oAction) {
					...
					if (!oItem.finType_finType || !oItem.rateType_rate || !oItem.currency ||
						(!oItem.fixedVal && isNaN(oItem.fixedVal))) {
						MessageBox.error(that.getText("msgInvalidValue"));
						return;
					}
					oItem.fixedVal = oItem.fixedVal.toString();
					that.getView().setBusy(true);
					oBinding.create(oItem);
					oObject.getModel(
						that.sModelName).submitBatch(that.aGroupID[0]);
