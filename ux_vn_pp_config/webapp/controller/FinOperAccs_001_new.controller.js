...
sap.ui.define([
	...
	return BaseController.extend("vn.pp.ux_vn_pp_config.controller.FinOperAccs_001_new", {

		/**
		 * Called when a controller is instantiated and its View controls (if available) are already created.
		 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
		 * @memberOf vn.pp.ux_vn_pp_config.view.FinOperAccs_001_new
		 */
		onInit: function () {
			//Groups ID a Actualizar el primero es el principal
			this.aGroupID = ["FinOperAccs_001_new", "FinOperAccs_001"];
			//Nombre modelo a actualizar
			this.sModelName = "modeloConfig";
			//Nombre de tablas a actualizar
			this.sObjectName = "listFinOperAccs_001_new";
		...
		onSave: function (oEvent) {
			...
			MessageBox.confirm(that.getText("msgConfirmTxt"), {
				...
				onClose: function (oAction) {
					...
					if (!oItem.sapVKORG || !oItem.sapSEGMENT ||
						!oItem.sapFINOPER || !oItem.sapHKONT || !oItem.sapKOSTL ||
						isNaN(oItem.sapHKONT) || isNaN(oItem.sapKOSTL)) {
						MessageBox.error(that.getText("msgInvalidValue"));
						return;
					}
					oItem.sapSEGMENT = oItem.sapSEGMENT.padStart(10, "0");
					oItem.sapHKONT = oItem.sapHKONT.padStart(10, "0");
					oItem.sapKOSTL = oItem.sapKOSTL.padStart(10, "0");
